...
import { ExternalLink, Github, Linkedin, Download, Server, Brain, Layers } from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-16">
          {projects.map((project, index) => (
            ...
            >
              <div className={`w-16 h-16 bg-gradient-to-r ${project.gradient} rounded-xl flex items-center justify-center mb-6 icon-btn-pop`}>
              ...
              <div className="flex flex-wrap gap-2">
                {project.tech.map((tech) => (
                  <span
                    key={tech}
                    className="px-3 py-1 bg-gray-800/50 text-cyan-400 rounded-full text-sm border border-cyan-500/20 btn-pop cursor-pointer"
                  >
                    {tech}
                  </span>
                ))}
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {links.map((link, index) => (
            <a
              ...
              className={`bg-gray-900/50 backdrop-blur-sm rounded-2xl p-8 border ${link.borderColor} social-btn-pop group block`}
            >
              <div className="flex items-center justify-between mb-4">
                <link.icon className={`w-8 h-8 ${link.color} icon-btn-pop`} />
